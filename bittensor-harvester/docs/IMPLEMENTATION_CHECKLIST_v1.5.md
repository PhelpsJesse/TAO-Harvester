# Implementation Checklist (v1.5 Controlled)

Date: 2026-03-17  
Baseline: `docs/REQUIREMENTS_SPEC_v1.5.md`  
Lessons Source: `docs/LESSONS_LEARNED_PRESERVED_2026-03-10.md`

## How to Use This Checklist

- This checklist is execution guidance under the v1.5 requirement baseline.
- Do not implement items that conflict with v1.5 without a documented requirement update first.
...
- [ ] Unattended Tier 1 runs always end in explicit `completed`, `failed`, or `manual_intervention_required` state.
- [ ] Tier 1 unattended run path is agent-driven on VPS OpenClaw runtime.
- [ ] OpenClaw unattended path is limited to low-security stages (ingest/reconcile/harvest planning intents) only.
- [ ] OpenClaw unattended path cannot execute TAO wallet-to-wallet transfers or TAO-to-USD sales.
- [ ] Retries are bounded and cannot loop indefinitely under repeated 429/upstream failure conditions.
- [ ] Fail-closed path does not emit partial harvest plans as success.
- [ ] Debug offramp exists via persisted run state, actionable error message, and rerunnable command path.
...
- [ ] Add explicit rate-budget behavior for backfill/high-fanout calls.
- [ ] Abort with actionable operator guidance when upstream data cannot be fetched safely.
...
- [ ] Add unattended-runtime outcome classification suitable for OpenClaw automation.
- [ ] Ensure reruns after failure resume or restart deterministically without duplicate side effects.
...
- [ ] Rate-limit tests covering bounded retries, fail-fast behavior, and no partial-success outputs.
- [ ] Resume/retry tests covering unattended failure then rerun from checkpoint.
...
Reference: Section 25 + pending v1.5 clarifications.
