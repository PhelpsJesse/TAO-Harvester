...
## Source Baseline (User-Provided v1.3, Amended to v1.5)
...
FR-011
Tier 1 operation MUST support unattended execution via an OpenClaw agent
running on a VPS for low-security workflow stages only (ingestion,
reconciliation, and harvest planning/intents). This unattended path MUST
NOT execute TAO wallet-to-wallet transfers or TAO-to-USD selling.
...
Unattended OpenClaw agents in Tier 1 MUST be restricted to these
low-security responsibilities and MUST NOT perform TAO wallet transfers
or TAO-to-USD sale execution.
